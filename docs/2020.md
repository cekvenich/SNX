...
## #1: Starter enterprise cloud software arcitecture: Grand Prix 

In Chess, there is a library of named openings, such as Queens Gambit. The opening, first 10 scripted moves or so, is played very quickly, and after it is on the board, the middle game starts where the moves are allways different. There is a chess opening that is frequently played by international grand masters, informally called 'Gand Prix', (a line that 
is a bit hard for chess n00bs, but it does confuse GMs to make them think you are a 2K rated player). 

There is a baseline enterprise cloud software arcitecture, I like to call it Grand Prix.  From that baseline Cloud Software Arcitecture, there are many variations, for example based on which of the top 8 cloud
providers you chose. (I recommend multi-cloud strategy). But in general, at hight level, it looks like implementation varations of this theme(drawn using Lucidchart):
...
You have an http server(eg. ngnix), app server (eg. Tomcat), and 2 DB servers(eg. MySQL). Two becuase you write to one DB, that replicates to the second DB - and you mostly read from the replicated server.

###  Grand Prix 

Again, there are 3 main parts, this time they are: the cloud **data storage** section of the arcitecture - on top left, **analytics** section of the arcitecture - on the botton, and the **app** section of the arcitecture - on the right.<br />
NOTE: I list vendors only to illustrate and clarify, I'm sure your favorite vendor is the best one. 

#### Data Storage section

- Here we have Object storage (in petabytes or more), typically it is S3/Hadoop due to fast speed and low cost. (there are dozens of S3 vendors, eg. Vultr).<br />
While S3 is fast, you can make it faster by using caching/queuing in the app part, or having the app access the analytics DB. The other benefit of object store 
is faster development iteration, since there is no schema. You can just write JSON lists to S3. Likey you'd have more than one object storage service to be regulation compliant. For example store
EU data in EU located object storage and American data in an cloud prodivder location inside USA.

- And the other main part is an events provider, so that we can implement some variation of an event driven architecture. (there are many vendors, eg. PubNub)<br />
I won't go into varations of variation of an event driven architecture but there are best pratices: the event is small in size, just a notification, you can go to S3 to get the data.
And you must build in governors, such as debounce or such.

- Also, here we have our logs. (eg: ELK by Logzio). Use of logs includes debugging and error monitoring.

Above section is mostly operated by apps developer (aka devops).

You can use other variations of the data storage, but you should try to limit yourself to cloud and service providers. 

#### Analytics section

Analytics section requires cooler heads to architect due to hype (related to AI, ML, etc.). Ideally someone who has helped implemeneted a statisical or a predictive system in the past should be responsible for this area.
The risk is that you spend $ and the only business uses of it is a press release hyping your company's anayltics(you could just issue a $50 press release and save the analytics implemenation budget).

There are may part of Grand Prix analytics section:
...
The RAM memory SQL is populated in nearl real time by the S3 object store from the data storage (Hello ETL. This process is mosly developed by the apps team. Mostly.) 

- Reporting vendor (eg: Sigma Computing, Excel). This would include KPI Dashboard.

- Network security(eg. DMZ) arround the entire anayltics section.<br />
For example ( lets see how many buzz-words I can pack in a single sentence:) A VPN, that is sometimes also a 'Bastion', to access a NAT based VPC. 
Depending on the scale, you may need a Netops/Linux security resource (This part can be implemented using iptables, after all, all the netops security 'boxes' are just linux apps).
While you could put network security that includes all, it does not add value and may get in the way - so just do secure the analyticis section.
If you are not worried about data security, don't invest in it, skip this part.

- Analytics section may also include 'bots' (eg. Puppeteer ). They are used for scraping external data (eg. SEO rankings), and more.

- Also, performance monitoring of your UX is in the anayltics section arcitecture. <br />
You may for example push a button to add/remove K8/Docker instances in the apps section. 

Depneding on scale, you may need at least 3 FTE for above: Dashboard maintainer(*), DBA and Netops security.
( If you are a SME or don't have a tech team, you like would need to leverage some anayltics-as-a-service vendor that does not do enterprise customers)

(*) Dashboard/reports needs to be updated as the markets and business changes. You can't just develop a dashboard|report and fire all the developers, you need to keep at least 1.


#### App section

The app section is the most well understood section by the developers, it has these parts:


- Front end(FE) app. Outside of the scope of this write up. But a few things to chew on: Consider looking at JAMstack and cross platfrom development. The hard part of FE development is naming css classes in a way that facilitates maintanance. FE is disposable and needs a rewrite yearly to match the changing market needs. The FE team is frequently remote(hello screen share). Desingers that use CSS are hard to hire.

- Client side API and client side ViewModel. This is not writen by FE developers, but by back end team even if it runs on the client side in the user application|web page. Below  #8 explains. The major requiremnet is: topology aware client.
as well as E2E testing, auth, etc. This may involve SEO, so both back end team and FE team together on SEO rankings - if one of your apps needs such.

- One example of FE app UI is: voice apps. Just don't forget that in your architecture. Just becuase you have a computer that is not seen, does not mean it's not there.

- The FE|Client apps talk to CDN. CDN provides HTTPs offloarind and early TLS termination. And it hides the IP of the 'REST' services, makeing them harder to attack. Don't use HTTP servers anymore, use the CDN and other edge features.
Again: there are no HTTP servers such as NGNX in an enterprise cloud software arcitecture. CDN takes 30 seconds to learn. (eg: CDN77). Also CDNs provide HTTP3, udp based. CDN's handle the TLS certs, so 
please avoid certs and TLS and HTTPS. And a nice thing CDN provide is: caching. We set the cache in the header of each request. This siginficaly simplifies development and reduces operating costs. For example set cache on a REST API to 2 seconds. 

- HMI Micro Service containers(Docker). CDN, trought the firewall, accesses the load balanced stateless services. (services include serving static assets like HTML, css, PNG, video). The client API's and ViewModel talks to these services. These service containers are GEO distributed, at minimum EU and Americas data centers, but possibly more granular. The service containers are HMI - high memory, for example 512G of RAM or more.
They would use local Redis memory or local SQLite memory for things like queuing and to have localy things that are needed to operate the service. We try to avoid network access by having the data in the application RAM. You can ofcourse 'warm up' the container by accessing your data store and analytics, and have a backround thread that keeps it fresh. 
<br />
IMPORTANT: Micro services are hard. They are meant to be used to allow independent scaling: we add containers to increase scale. This obviously does not work if different microservices are expeincing the same backpressure of the same downstream service. So to do micro services are an application arcitecture. I just labled it as micro service - but it's not, unless someone engineers a design to allow indedpenedet scaling. 
This is often forgoten, and the migrations to continerized architevute hence prodives no value. You know you are doing it wrong if you design took only days to come up with and did not include stress testing to 
verify the hypotesis. Organiations that don't do stress testing tend to have an applicaiton arcitevure designed by whoever talks loudests. People don't like to work there. 

So that is it for high level Enterprise cloud software arcitecture: Grand Prix. 

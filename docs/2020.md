...
In Chess, there is a library of named openings, such as Queens Gambit. The opening, first 10 scripted moves or so, is played very quickly, and after it is on the board, the middle game starts where the moves vary. There is a chess opening that is frequently played by international grand masters, informally called 'Gand Prix'.
...
is faster development iteration, since there is no schema. You can just write JSON lists of rows to S3. Likely you'd have more than one object storage service locations, to be regulation compliant. For example store
EU data in EU located object storage and American data in an cloud provider location - inside the USA.
...
I won't go into variations of event driven architectures, but there are some best practices to consider: the event should be small in size - just a notification, you can go to S3 to get the data.
And you must build in governors in event generators, such as debounce. Simplitically, micro services generate an event on a DB write so that analytics get updated. (and async, analytics generates an event that it updated so micro services get the new segment predictions in their local RAM)

- If you are Kafka volume level, then use Kafka in front of S3. (eg. Instaclustr)

- Also, here we have our app logs. (eg: ELK by Logzio). Use of logs includes debugging and error monitoring.

- And if there is a Blockchain Hyperledger use case, that  is in this section.

<p/>
Above section is mostly operated by apps developer (aka devops) and may not require FTE operator.
...
The RAM memory SQL DB is populated in near real time from the S3 object store (Hello ETL) This process is mostly developed by the apps team. Mostly.
 
- Reporting vendor (eg: Sigma Computing, Excel). This would include KPI Dashboard and preditive models
...
Performance monitoring of your UX is usually in the analytics section architecture. <br />
You may for example push a button, to add/remove K8/Docker instances in the apps section via K8(Kubernetes). 
...
- Front end(FE) app. FE is outside of the scope of this write up. But a few things to chew on regarding FE: Consider looking at JAMstack, low code and cross platform development. The hard part of FE development is naming css classes in a way that facilitates maintenance. FE is disposable and needs a updated yearly to match the changing market needs. The FE team is frequently remote. Designers that use CSS are hard to hire. Good luck to FE devs, often it is harder than the back end.
...
One example of FE app UI is: voice apps. Don't forget that in your architecture. Just because you have a computer that is not seen, does not mean it's not there.
...
<br />
IMPORTANT: Micro services are hard. They are meant to be used to allow independent scaling: we add containers to increase scale. This obviously does not work if different micro services are turtlenecked by the same back-pressure of the same downstream service. Micro services are an application architecture. I just labeled it as micro service - but it's not a micro service, unless someone engineers a design to allow independent scaling. You could still have a monolithic application architecture in different containers. You know you are doing it wrong if you design took only days to come up with and did not include stress testing to 
verify the design hypothesis. 
<br />
...
Our new RAM friends include Aerospkie, Spark, Clickhouse, SAP HANA, Apache Ignite, Apache Arrow, etc. 
...
In addition to DB clusters, it is now common for the RAM DB to also be local in your application's container. A 'fat memory' container w/ 512Gig , that reduces network latency, adapting the Spark architecture. Your cloud VM's should be 512 Gig or RAM or more, where each Docker container also has local Redis or SQLite. And if you don't properly configure your cloud containers you will end up overpaying for your cloud. I think we all know  how much profit AWS generates for Amazon. Don't be the organization that facilitates their margins.
...
And, before the '20s we started deploying apps to the cloud. And now even our IDE can be in the cloud as well, check out CodeAnywhere or similar for you development environment.
...
Note: Topology aware client enbales scale, if that is a requirment. 
<p />

Of course, same goes for IOS(Swift)) and Android(Kotlin) apps. Back end/data engineering team  must be polyglot and write and support the ViewModel and API calls cross platform and document it. And still support the raw back end service.

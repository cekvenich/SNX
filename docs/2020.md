...
In Chess, there is a library of named openings, such as Queens Gambit. The opening, first 10 scripted moves or so, is played very quickly, and after it is on the board, the middle game starts where the moves are vary. There is a chess opening that is frequently played by international grand masters, informally called 'Gand Prix'.

There is a baseline enterprise cloud software arcitecture, I like to call it Grand Prix.  From that baseline cloud software architecture, there are many variations, for example based on which of the top 8 cloud
...
You have an http server(eg. NGINX), app server (eg. Tomcat), and 2 DB servers(eg. MySQL). Two becuase you write to one DB, that replicates to the second DB - and you mostly read from the replicated server.

###  Grand Prix 

There are 3 main sections again, this time they are: the cloud **data storage** section of the arcitecture - on top left, **analytics** section of the arcitecture - on the botton, and the **app** section of the arcitecture - on the right.<br />
...
is faster development iteration, since there is no schema. You can just write JSON lists of rows to S3. Likey you'd have more than one object storage service locations, to be regulation compliant. For example store
EU data in EU located object storage and American data in an cloud prodivder location - inside the USA.

- The other main part is an events provider, so that we can implement some variation of an event driven architecture. (there are many vendors, eg. PubNub)<br />
I won't go into varations of event driven architectures, but there are some best practices to consider: the event should be small in size - just a notification, you can go to S3 to get the data.
And you must build in governors in event generators, such as debounce.
...
The risk is that you spend $ and the only business uses of it is a press release hyping your company's analytics.
...
The RAM memory SQL DB is populated in nearl real time from the S3 object store (Hello ETL) This process is mosly developed by the apps team. Mostly.
...
Depending on the scale, you may need a Netops/Linux security FTE. This part can aslo be implemented using iptables; after all, all the netops security 'boxes' are just linux apps.
...
Performance monitoring of your UX is usually in the anayltics section arcitecture. <br />
You may for example push a button to add/remove K8/Docker instances in the apps section. 

#### App section

The app section is the most well understood section by the developers, it has all these parts:


- Front end(FE) app. FE is outside of the scope of this write up. But a few things to chew on regarding FE: Consider looking at JAMstack and cross platfrom development. The hard part of FE development is naming css classes in a way that facilitates maintanance. FE is disposable and needs a updated yearly to match the changing market needs. The FE team is frequently remote. Desingers that use CSS are hard to hire. Good luck to FE devs, often it is harder than the back end.

- Client side API and client side ViewModel. This is not writen by FE developers, but by back end team. Even if it runs on the client side,  inside
the user application|web page, back end team should implement it.Below in this article: #8 explains more on this point. The major requiremnet of the client API: topology aware client.
...
One example of FE app UI is: voice apps. Don't forget that in your architecture. Just becuase you have a computer that is not seen, does not mean it's not there.

- CDN that the he FE|Client apps talk to. CDN provides HTTPs offloarind and early TLS termination. And it hides the IP of the 'REST' services, makeing them harder to attack. Also CDNs provide HTTP3, udp based. CDN's handle the TLS certs, so   And a nice thing CDN provide is: caching. We set the cache in the header of each request. This significantly simplifies development and reduces operating costs. For example set cache on a REST API to 2 seconds.  Don't use HTTP servers anymore(eg: NGNX), use the CDN and other edge features.(eg: CDN77).  CDN is one of the big wins on migrating to the cloud: it is cheaper and faster. 


- HMI Micro Service in containers(Docker). CDN, trought the firewall, accesses the load balanced stateless services. (services include serving static assets like HTML, css, PNG, video, it is a file server behind a CDN). The client API's and ViewModel talks to these services. These service containers are GEO distributed, at minimum EU and Americas data centers, but possibly more granular. The service containers are HMI - high memory, for example 512G of RAM or more. Section #4 in this write up touches on it.
They would use local Redis memory or local SQLite memory for things like queuing and to have localy things that are needed to operate the service. We try to avoid network latency of services by having the data in the application RAM. You of course 'warm up' the container by accessing your data store and analytics data, and have a backround daemon thread that keeps local data fresh. 
<br />
IMPORTANT: Micro services are hard. They are meant to be used to allow independent scaling: we add containers to increase scale. This obviously does not work if different microservices are turtlenecked by the same backpressure of the same downstream service. Micro services are an application arcitecture. I just labled it as micro service - but it's not a micro service, unless someone engineers a design to allow indedpenedet scaling. You could still have a monolithic application architecture in different containers. You know you are doing it wrong if you design took only days to come up with and did not include stress testing to 
verify the design hypotesis. 
